...
sed -i "s/v=[0-9.]*\"/v=$TIMESTAMP\"/g" ~/escort/client/public/index.html
...
# Обновляем временную метку в скопированных файлах
log "🔄 Обновление временной метки в файлах после копирования"
sed -i "s/content=\"[0-9.]*\"/content=\"$TIMESTAMP\"/g" ~/escort/server/public/index.html
sed -i "s/v=[0-9.]*\"/v=$TIMESTAMP\"/g" ~/escort/server/public/index.html
